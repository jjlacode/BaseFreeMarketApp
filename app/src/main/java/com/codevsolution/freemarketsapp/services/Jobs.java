...
public class Jobs extends JobService implements JavaUtil.Constantes, Interactor.ConstantesPry, ContratoPry.Tablas {
    ...
    public boolean onStartJob(JobParameters params) {
        ...
        if (ahora > time0 && CheckPermisos.validarPermisos(this, CheckPermisos.WRITE_EXTERNAL_STORAGE, 100) &&
                CheckPermisos.validarPermisos(this, CheckPermisos.READ_EXTERNAL_STORAGE, 200)) {
            ...
            if (ahora < timeStamp + (MINUTOSLONG * minutosCopia)) {
                ...
                try {
                    if (SQLiteUtil.BD_backup(null, true) &&
                            SQLiteUtil.BD_backup(SYSTEM, idUser, true)) {
                        System.out.println("Creada copia de bd instantanea");
                        time0 = ahora + (MINUTOSLONG * (minutosCopia - 1));
                        AndroidUtil.setSharePreference(getApplicationContext(), PREFERENCIAS, TIMESTAMP, 0L);
                    } else {
                        System.out.println("Falló la copia de seguridad instantanea");
                        intentos++;
                        if (intentos >= 10) {
                            intentos = 0;
                            time0 = ahora + (MINUTOSLONG * (minutosCopia - 1));
        ...
        if (ahora > time1 && c.get(Calendar.HOUR_OF_DAY) == 0 &&
                CheckPermisos.validarPermisos(this, CheckPermisos.WRITE_EXTERNAL_STORAGE, 100) &&
                CheckPermisos.validarPermisos(this, CheckPermisos.READ_EXTERNAL_STORAGE, 200)) {
