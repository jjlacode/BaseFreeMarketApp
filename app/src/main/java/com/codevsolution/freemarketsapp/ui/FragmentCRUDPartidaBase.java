...
public class FragmentCRUDPartidaBase extends FragmentCRUD implements Interactor.ConstantesPry,
    ...
    private double calcularPrecio(String id) {
        ...
        double precio = 0;
        ListaModeloSQL listadet = setListaModeloDetalle(CAMPOS_DETPARTIDABASE, id);
        for (ModeloSQL detPartida : listadet.getLista()) {

            if (detPartida.getString(DETPARTIDABASE_TIPO).equals(TIPOTRABAJO)) {
                ModeloSQL trabajo = updateModelo(CAMPOS_TRABAJO, detPartida.getString(DETPARTIDABASE_ID_DETPARTIDABASE));
                ...
                ModeloSQL producto = updateModelo(CAMPOS_PRODUCTO, detPartida.getString(DETPARTIDABASE_ID_DETPARTIDABASE));
    ...
    private void calcularPrecioProdProv(final String id) {
        ...
        final ListaModeloSQL listadet = setListaModeloDetalle(CAMPOS_DETPARTIDABASE, id);
            ...
            Thread th = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (nn(secPartida) && secPartida > 0) {
                        actualizarRegistro(TABLA_PARTIDA, idPartida, secPartida, valores);
                    } else {
                        ...
                        ModeloSQL proyecto = updateModelo(CAMPOS_PROYECTO, idPartida);
                        putDato(valores, PARTIDA_ID_ESTADO, proyecto.getString(PROYECTO_ID_ESTADO));
                        System.out.println("valores = " + valores);

                        secPartida = crearRegistroSec(CAMPOS_PARTIDA, idPartida, valores);
                    }

                    ModeloSQL partida = updateModelo(CAMPOS_PARTIDA, idPartida, secPartida);
                    String iddetpartida = partida.getString(PARTIDA_ID_PARTIDA);
                    ListaModeloSQL listaDetPartidabase = setListaModeloDetalle(CAMPOS_DETPARTIDABASE, id);
                    for (ModeloSQL detPartidaBase : listaDetPartidabase.getLista()) {
                        ...
                        ListaModeloSQL listaDetPartida = setListaModeloDetalle(CAMPOS_DETPARTIDA, iddetpartida);
                        for (ModeloSQL detPartida : listaDetPartida.getLista()) {
                            ...
                            if (detPartida.getString(DETPARTIDA_ID_DETPARTIDA).equals(detPartidaBase.getString(DETPARTIDABASE_ID_DETPARTIDABASE))) {
                                actualizarRegistro(TABLA_DETPARTIDA, iddetpartida, detPartida.getInt(DETPARTIDA_SECUENCIA), valores);
                                detnuevo = false;
                            }
                        }
                        if (detnuevo) {
                            crearRegistroSec(CAMPOS_DETPARTIDA, iddetpartida, valores);
                    ...
                    for (ModeloSQL detPartida : listaDetPartida.getLista()) {
                        ...
                        if (cambio) {
                            borrarRegistro(TABLA_DETPARTIDA, detPartida.getString(DETPARTIDA_ID_PARTIDA), detPartida.getInt(DETPARTIDA_SECUENCIA));
                        }
                    }
        ...
        builder.setItems(opciones, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (opciones[which].equals("Clonar partida base")) {
                    ...
                    if (nn(id)) {
                        actualizarRegistro(tabla, id, valores);
                        partidabase = updateModelo(campos, id);
                    } else {
                        Uri uri = crearRegistro(tabla, valores);
                        partidabase = consultaBD.queryObject(campos, uri);
                    ...
                    ArrayList<ModeloSQL> listaclon = consultaBD.queryListDetalle
                    ...
                    for (ModeloSQL clonpart : listaclon) {
                        ...
                        crearRegistroDetalle(CAMPOS_DETPARTIDABASE, id, valores);
                    }
                    nuevo = false;
                    selector();
    ...
    class ViewHolderDetPartida extends BaseViewHolder implements TipoViewHolder {
        ...
        public void bind(final ModeloSQL modeloSQL) {
            ...
            if (tipodetpartida.equals(TIPOTRABAJO)) {
                ...
                ModeloSQL trabajo = updateModelo(CAMPOS_TRABAJO, idDetPartidaBase);
                gone(cantidad);
                ...
                ModeloSQL producto = updateModelo(CAMPOS_PRODUCTO, idDetPartidaBase);
    ...
    private void setAdaptadorAuto(AutoCompleteTextView autoCompleteTextView) {
        ...
        ListaModeloSQL lista = setListaModelo(campos);
